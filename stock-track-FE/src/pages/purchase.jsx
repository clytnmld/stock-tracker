          ...
          <ul>
            {products.map((item) => (
              <li key={item.id}>
                ...
                <Link to={`${item.id}`}>
                  <button style={{ margin: "20px" }}>Purchase</button>
                </Link>
              </li>
            ))}
          </ul>
...
export function DoPurchase() {
  ...
  const [purchase, setPurchase] = useState(purchaseFormat);
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(BASE_URL_API + `purchase/${productId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
          ...
          <p>
            Available stock:{" "}
            {
              product.productStock.find(
                (s) => s.warehouseId === parseInt(purchase.warehouseId)
              )?.stock ?? 0
            }
          </p>
        )}
